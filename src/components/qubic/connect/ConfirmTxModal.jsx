...
const ConfirmTxModal = ({
  ...
  useEffect(() => {
    if (tick !== null && confirmedTx !== null && initialTick !== null) {
      ...
      const progress = ((tick - initialTick) / (targetTick - initialTick)) * 100;
  ...
  useEffect(() => {
    if ((transactionStatus === 'success' || transactionStatus === 'failure') && open) {
      const timeoutId = setTimeout(() => {
        onClose();
        // Reset
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (confirmed && confirmed.targetTick) {
        ...
        throw new Error('Transaction not correctly confirmed (targetTick is missing).');
      }
    } catch (error) {
      ...
      >
        ...
        >
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Typography variant="h6" color={theme.palette.text.primary} sx={{ fontWeight: 'bold' }}>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={1}>
            {(isBet || isCreate || isPublish) &&
              !['pending', 'failure', 'success'].includes(transactionStatus) && (
                <Box display="flex" alignItems="center" justifyContent="center" my={1}>
                  <WarningAmberIcon color="info" sx={{ fontSize: 28, mr: 1 }} />
                  <Typography variant="h7" align="center" sx={{ fontWeight: 500, display: 'flex' }}>
                    {tx.description}
                    <Box
                      component="span"
                      sx={{
